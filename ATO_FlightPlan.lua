...
versionDCE["ATO_FlightPlan.lua"] = "1.58.280"
------------------------------------------------------------------------------------------------------- 
...
-- modification M61_j		SAR (j bug parkAlertSAR.occupied)(debug SAR on CV)(f radio_start)(d theatre)
...
for basename, base in pairs(db_airbases) do															--iterate through airbases
	if base.ATC_frequency and base.ATC_frequency ~= "" and type(base.ATC_frequency)~= "table" then		
		-- assigned_freq[tonumber(base.ATC_frequency)] = true
	...
	elseif base.ATC_frequency and type(base.ATC_frequency)== "table" then
		for n , freq in ipairs(base.ATC_frequency) do	
			-- assigned_freq[tonumber(freq)] = true
...
--testd_
function  createBombingChapter(id_task ,flight ,waypoints, weaponType, attackType, attackAlt, element, from, typeCible )
			...
			for f = 1, #flight do												--iterate through flights in roles
				...
				if (flight[f].route[1].id ~= "Spawn" and flight[f].route[1].eta and flight[f].route[2]) or (flight[f].route[1].id ~= "Spawn" and flight[f].route[1].eta and not db_airbases[flight[f].base].unitname) then										
					...
					if (db_airbases[flight[f].base].LimitedParkNb and TabLPark[flight[f].base][timmingParking]  )    then						 --or TabLPark[flight[f].base].flag										
						...
						if not TabLPark[flight[f].base]["LimitedParkNb"] then 
						...
						if (flight[f].task == "Intercept" or flight[f].task == "SAR" )  and not db_airbases[flight[f].base].unitname then
							if  db_airbases[flight[f].base].LimitedParkNb then
								db_airbases[flight[f].base].LimitedParkNb = db_airbases[flight[f].base].LimitedParkNb - flight[f].number
				...
				local groupTaskTemp = flight[f].task
				...
				elseif flight[f].task == "Anti-ship Strike" then 
					groupTaskTemp = "Antiship Strike"											-- Miguel debugB 
				elseif flight[f].task == "SAR" or flight[f].task == "CSAR"  then 
					groupTaskTemp = "Transport"													-- Miguel debugB 
				elseif flight[f].task == "Runway Attack"  then 
					groupTaskTemp = "Runway Attack"
				end
				...
				local goodTask = true
				-- if groupTaskTemp == "CAS" or groupTaskTemp == "Pinpoint Strike"  or groupTaskTemp == "Ground Attack"  or groupTaskTemp == "Runway Attack" then

					-- print("AtoFP passe #1 typeCible "..typeCible.."| |"..flight[f].type.."| |"..groupTaskTemp.."| |"..tostring(flight[f].target.name))
					
					local foundGoodTask = false
					local id_task = ""
					if not  TaskByPlane[groupTaskTemp][flight[f].type] or not groupTaskByTypeTarget[typeCible][groupTaskTemp]  then
						
						-- print("AtoFP  if not  TaskByPlane[groupTaskTemp][flight[f].type]  "..tostring(groupTaskTemp))

						goodTask = false
					
						--iteration de la table avion/task possible
						for groupTaskTemp_, plane in pairs(TaskByPlane) do							
							for type_, value in pairs(plane) do 								
								if type_ == flight[f].type then									
									--iteration de la table id/task possible
									for task_, idStrike in pairs(groupTaskByTypeTarget[typeCible]) do										
										if groupTaskTemp_ == task_ then										
											for idStrike_, value2 in pairs(idStrike) do												
												if idStrike_ and idStrike_ ~= nil then 													
													if flight[f].task == "Strike" or  flight[f].task == "Antiship Strike" then
														if task_ == "Antiship Strike" and groupTaskTemp == "Ship" then
															-- print("AtoFP  groupTaskTemp avant "..tostring(groupTaskTemp))
															
															id_task = idStrike_														
															groupTaskTemp = task_
															
															-- print("AtoFP Ship après le groupTaskTemp après "..flight[f].type.." "..tostring(groupTaskTemp).." id_task: "..tostring(id_task))
															-- os.execute 'pause'
															goodTask = true
															breakloop = true
															break
															
														elseif task_ == "Runway Attack" and groupTaskTemp == "Runway Attack" then
															-- print("AtoFP  groupTaskTemp avant "..tostring(groupTaskTemp))
															
															id_task = idStrike_															
															groupTaskTemp = task_
															
															-- print("AtoFP Runway après groupTaskTemp après "..flight[f].type.." "..tostring(groupTaskTemp).." id_task: "..tostring(id_task))
															-- os.execute 'pause'
															goodTask = true
															breakloop = true
															break
														elseif not (task_ == "Runway Attack" or  task_ == "Antiship Strike" )then															
															...
															-- print("AtoFP  groupTaskTemp après "..flight[f].type.." "..tostring(groupTaskTemp).." id_task: "..tostring(id_task))
															-- os.execute 'pause'
															goodTask = true
															breakloop = true
															break
						...
						for task_, idStrike in pairs(groupTaskByTypeTarget[typeCible]) do							
							for idStrike_, value2 in pairs(idStrike) do
								if idStrike_ and idStrike_ ~= nil then
									if groupTask[groupTaskTemp] and  groupTask[groupTaskTemp][idStrike_] then
										
										id_task = idStrike_
										
										-- print("AtoFP #12 FOUND choix  |"..tostring(groupTaskTemp).."| |"..tostring(id_task))
										-- os.execute 'pause'
						...
						if not breakloop then 
							-- print("AtoFP #14 choix  "..tostring(groupTaskTemp).." id_task "..tostring(id_task).." typeCible "..tostring(typeCible))
							-- os.execute 'pause'
						end
					end	
				...
				if not goodTask then
					if groupTaskTemp ~= "Runway Attack" then
						print("(downgraded mode  : don t found task "..flight[f].task.." of the "..flight[f].type.. "in the oob_air_init.lua file")
						print("typeCible "..typeCible.." || "..flight[f].type.." "..groupTaskTemp.." "..tostring(flight[f].target.name))
						-- os.execute 'pause'
					end
					...
					if  TaskByPlane["Ground Attack"][flight[f].type]   then											
						...
						id_task = "Bombing"
						goodTask = true
						-- print("AtoFp passe BA "..groupTaskTemp)
					else
						...
						for groupTaskTemp_, plane in pairs(TaskByPlane) do							
							for type_, value in pairs(plane) do 								
								if type_ == flight[f].type then	
									...
									if groupTaskTemp_ == "CAS" or groupTaskTemp_ == "Pinpoint Strike" or  groupTaskTemp_ == "Antiship Strike"  or groupTaskTemp_ == "Runway Attack" then
										-- print("AtoFp passe BE ")
										
										groupTaskTemp = groupTaskTemp_
										id_task = "Bombing"																			
										for id_task_ , value in pairs(strikeDegradedMode[groupTaskTemp]) do
											id_task = id_task_
											goodTask = true
				...
				-- print("AtoFP passe #2 typeCible "..typeCible.." || "..flight[f].type.." "..groupTaskTemp.." "..tostring(flight[f].target.name))
				...
				for w = 1, #flight[f].route do
						...
						local task_entry = {	
							...
							["params"] = 
							{
								["action"] = 
								{
									...
									["params"] = 
									{
										["command"] = "OrbitPosition('" .. grpname .. "', " .. waypoints[w]["alt"] .. ", " .. flight[f].loadout.vCruise  / 4 * 3 .. ", " .. (waypoints[w].ETA + 300) .. ")",
									},
								},
							},
						}
												...
												[1] = {
													...
													["params"] = 
													{
														["action"] = 
														{
															...
															["params"] = 
															{
																["command"] = "OrbitPosition('" .. grpname .. "', " .. waypoints[w-1]["alt"] .. ", " .. waypoints[w-1]["speed"] .. ", " .. departure_timeB .. ")",
															},
														},
													},
												},
					...
					if  flight[f].route[w].id == "Attack" then	
						...
						-- 					["params"] = 
						-- 					{
						-- 						["command"] = "CustomMapObjectAttack('" .. grpname .. "', {" .. tgtlist .. "}, '" .. ExpendQty .. "', '" .. weaponType .. "', '" .. attackType .. "', '" .. attackAlt .. "', '" .. groupTaskTemp ..  "')",	--this is a custom written task to allow all aircraft in flight to attack multiple static objects simultenously
						-- 					},
						-- 				},
						-- 			},
						-- 		}
						...
						-- 		-- print("AtoFp flight[f].task: "..flight[f].task.." groupTaskTemp: "..tostring(groupTaskTemp))
						...
						-- 						["params"] = 
						-- 						{
						-- 							["command"] = "CustomGroupAttack('" .. grpname .. "', '" .. flight[f].target.name .. "', '" .. ExpendQty .. "', '" .. weaponType .. "', '" .. attackType .. "', '" .. attackAlt .. "', '" .. groupTaskTemp ..  "')", 
						-- 						},
						-- 					},
						-- 				},
						-- 			}
									...
									local task_entry = {																				--task is a command to run LUA code
										...
										["params"] = 
										{
											["action"] = 
											{
												...
												["params"] = 
												{
													["command"] = "CustomMixClassAttack('" .. grpname .. "', {" .. tgtlist .. "}, '" .. ExpendQty .. "', '" .. weaponType .. "', '" .. attackType .. "', '" .. attackAlt .. "', '" .. groupTaskTemp ..  "')",	--this is a custom written task to allow all aircraft in flight to attack multiple static objects simultenously
												},
									...
									local task_entry = {																				--task is a command to run LUA code
										...
										["params"] = 
										{
											["action"] = 
											{
												...
												{
																--AirGroundAttackTask(FlightName,				 Target,						 WeaponType,string			 ExpendQty,string		 Dive,			 OffsetAngle, 			ClimbAngle, 		PopAlt, 		AttackDist, 			Reattack,			 Debug)
													["command"] = "AirGroundAttackTask('" .. grpname .. "', {" .. tgtlist .. "}, '" .. WeaponType_ .. "', '"  .. ExpendQty .. "', " .. tostring(Dive) .. ", " .. tostring(OffsetAngle) .. ", " .. tostring(ClimbAngle) ..", " .. tostring(PopAlt) ..", " .. tostring(AttackDist) ..", " .. tostring(Reattack) .. ", " .. tostring(DebugTask) ..  ")", 
												},
											},
										},
									}
								...
								local task_entry = {																				--task is a command to run LUA code
									...
									["params"] = 
									{
										["action"] = 
										{
											...
											["params"] = 
											{
												["command"] = "CustomAirbaseAttack('" .. grpname .. "', {x = " .. flight[f].target.x .. ", y = " .. flight[f].target.y .. "}, '" .. ExpendQty .. "', '" .. weaponType .. "', '" .. attackType .. "', " .. attackAlt .. ")",
											},
										},
									},
								}
									...
									local task_entry = {																				--task is a command to run LUA code
										...
										["params"] = 
										{
											["action"] = 
											{
												...
												{
																--AirGroundAttackTask(FlightName,				 Target,															 WeaponType,string			 ExpendQty,string		 Dive,			 OffsetAngle, 			ClimbAngle, 		PopAlt, 		AttackDist, 			Reattack,			 Debug)
													["command"] = "AirGroundAttackTask('" .. grpname .. "', {x = " .. flight[f].target.x .. ", y = " .. flight[f].target.y .. "}, '" .. WeaponType_ .. "', '"  .. ExpendQty .. "', " .. tostring(Dive) .. ", " .. tostring(OffsetAngle) .. ", " .. tostring(ClimbAngle) ..", " .. tostring(PopAlt) ..", " .. tostring(AttackDist) ..", " .. tostring(Reattack) .. ", " .. tostring(DebugTask) ..  ")", 
												},
											},
										},
									}
								...
								local task_entry = {																			
									...
									["params"] = 
									{
										["action"] = 
										{
											...
											["params"] = 
											{
												["command"] = "CustomMapObjectAttack('" .. grpname .. "', {" .. tgtlist .. "}, '" .. ExpendQty .. "', '" .. weaponType .. "', '" .. attackType .. "', '" .. attackAlt .. "', '" .. groupTaskTemp ..  "')",
												-- ["command"] = "CustomStaticAttack('" .. grpname .. "', {" .. tgtlist .. "}, '" .. ExpendQty .. "', '" .. weaponType .. "', '" .. attackType .. "', '" .. attackAlt .. "', '" .. groupTaskTemp ..  "')",	--this is a custom written task to allow all aircraft in flight to attack multiple static objects simultenously
											},
								...
								["params"] = 
								{
									["action"] = 
									{
										...
										["params"] = 
										{
											...
											["command"] = "Custom_SAR('" .. grpname .. "', '" .. flight[f].base .. "', '" .. db_airbases[flight[f].base].x .. "', '" .. db_airbases[flight[f].base].y .. "', '" .. flight[f].target_name .. "', '" .. flight[f].loadout.vCruise .. "', '" .. alt_cruise ..  "')", 
										},
									},
								},
							}
							...
							local task_entry = {	
								...
								["params"] = 
								{
									["action"] = 
									{
										...
										["params"] = 
										{
											["command"] = "CustomSearchThenEngage('" .. grpname .. "', " .. flight[f].loadout.standoff .. ", 'Air'," .. searchTime .. ")",
										},
									},
								},
							}
							...
							local task_entry = {																				--task is a command to run LUA code
								...
								["params"] = 
								{
									["action"] = 
									{
										...
										["params"] = 
										{
											["command"] = "CustomLaserDesignationAFAC('" .. grpname .. "', '" .. flight[f].target.x .. "', '" .. flight[f].target.y .. "',  '" .. flight[f].target.LaserCode .. "')",	
											-- ["command"] = "CustomLaserDesignation('" .. grpname .. "', '" .. tgt .. "', '" .. class .. "', '" .. flight[f].target.LaserCode .. "')",	--this is a custom written task to allow coordinates bombing of target poistion at time of attack
										},
									},
								},
							...
							local task_entry = {	
								...
								["params"] = 
								{
									["action"] = 
									{
										...
										["params"] = 
										{
											["command"] = "OrbitPosition('" .. grpname .. "', " .. altitude .. ", " .. speed .. ", " .. departure_time .. ")",
										},
									},
								},
							}
				...
				-- 				camp.client[flight[f].IdClient].tgt_wp = camp.client[flight[f].IdClient].tgt_wp - 1
				-- 			end
				-- 			break
				-- 		end
				-- 	end
				...
				if frequency[type_withData] and frequency[type_withData].radio.frequencyMustBeRadio1 then
					if not FreqCapability2(testFreqency, type_withData, 1, info) then
						...
						local name =  "Pack " .. p .. " - " .. flight[f].name .. " - " .. flight[f].task .. " " .. (f + addNflight)
						if frequency[type_withData].radio then
							for range, value in pairs(frequency[type_withData].radio) do
								local i=0
								repeat
									testFreqency = GetFrequency(side, name, flight[f].task, type_withData, range)
				...
				--======================================
				group['task'] = groupTaskTemp

				-- print("AtoFP task groupTaskTemp "..tostring(groupTaskTemp))
				...
				if debugStart then debugTxt_AtoFP = debugTxt_AtoFP.."\n"..("") end	
				if debugStart then debugTxt_AtoFP = debugTxt_AtoFP.."\n"..("AtoFP FirstMsg " .. "Pack " .. p .. " - " .. flight[f].name .. " - " .. flight[f].task .. " " .. (f + addNflight)) end
				...
				-- = - = - = - = -- = - = - = - = - = - = - = - = - = - = -- = - = - = - = - = - = - = - = - = - = -- = - = - = - = - = - = - = - = - = - = -- = - = - = - = - = - = -
				if (db_airbases[flight[f].base].unitname or LimitedParkTiming ) and  ( flight[f].player or flight[f].client ) and waypoints[1]["type"] ~= "Turning Point" then --??? LimitedParkTiming? vraiment pour le joueur?
				...
				if (( flight[f].task ~= "Intercept"  and flight[f].task ~= "SAR")  or LimitedParkTiming ) and  not flight[f].player and not flight[f].client and waypoints[1]["type"] ~= "Turning Point"
					...
					if (db_airbases[flight[f].base].unitname and (flight[f].task ~= "Intercept"  and  flight[f].task ~= "SAR"))  or string.find(flight[f].base,"FARP") then												--for groups on aircraft carriers -- modification M37.e SuperCarrier   and ((not SuperCarrier ) or (SuperCarrier and spawn_time>600))
						...
						if  (SinglePlayer  or  SingleWithDServer) and not SingleWithDServerAiAir then
							...
							if group['route']['points'][1]["ETA"]	<  (mission_ini.startup_time_player + 200) and waypoints[1]["action"] ~= "Turning Point" and not SpawnAir and not SpawnCata then
								...
								if counter == 20 or placeTiming < 0 then 
									...
									SpawnOn( "air", waypoints, group, Pn, spawn_time + 30, BugFrom, flight, f)
								else
							...
							if camp.player  and camp.player.side == side and camp.player.pack_n == p and camp.player.airbase == flight[f].base and flight[f].task ~= "AWACS" and flight[f].task ~= "Refueling" then					--for flights in player's package and package does not cover a station and flight[f].task ~= "CAP"
								...
								if not SpawnDeck then 
									...
									SpawnOn(data_configuration.SC_SpawnOn[flight[f].type], waypoints, group, Pn, spawn_time, BugFrom, flight, f)
								end
						...
						elseif   (Multi.NbGroup >= 1 and not camp.MultiPlayer.pack_n[p]) or SingleWithDServerAiAir  then	--en multiplayer: aucun décalage sur le pont, puisque tous les IA commencent en vol								
							...
							if not FARP_MorePlace then
								--les Planes qui genent le taxiing spawn selon conf_mod
								if not SpawnDeck or string.find(flight[f].base,"FARP") then 
									local BugFrom =  " not SpawnDeck "..debug.getinfo(1).currentline
									SpawnOn(data_configuration.SC_SpawnOn[flight[f].type], waypoints, group, Pn, spawn_time + 30, BugFrom, flight, f)
									if FlagInsertSixpack then																			--si le vol postulait pour le sixpack, on le supprime de la table
										table.remove(testSixPack[flight[f].base])
										FlagInsertSixpack = false
										for forTiming, value in pairs(TimingDeckCata[side][flight[f].base]) do
					...
					--SUR PISTE DUR---
					------------------	
					elseif (flight[f].task ~= "Intercept"  and  flight[f].task ~= "SAR") then 															
				...
				if flight[f].task == "SAR" and db_airbases[flight[f].base].unitname then 
					conditionUno = false
				...
				if (flight[f].task == "SAR" and conditionUno)  then			--or flight[f].task == "CSAR" 		
					...
					if ( flight[f].client ~= true and flight[f].player ~= true)  then	--or LimitedParkTiming or  ParkSarAirBase[flight[f].base]						-- M11 PVP ne copie pas de trigger retardé START pour les clients/joueurs	
						...
						-- and (not FARP_MorePlace and string.find(flight[f].base,"FARP")
						if (db_airbases[flight[f].base].unitname or LimitedParkTiming or db_airbases[flight[f].base].BaseAirStart) and not FARP_MorePlace  then
					...
					for n = 1, #group.route.points   do
						for baseName, airbase in pairs(db_airbases) do							
							if  (not airbase.inactive or airbase.inactive == false) and airbase["side"] == side and flight[f].base ~= baseName and not airbase.BaseAirStart and airbase.x then		--airbase.divert and					
								...
								if flight[f].type == "AV8BNA" or db_airbases[flight[f].base].unitname   then
								...
								elseif is_helicopter then
									...
									if  not (airbase.helipadId or string.find(baseName, "LHA") or airbase.unitname) then
										if distance < 300000 then
											local temp = {
												baseName = baseName,
												dist = distance
									...
									[1] = {
										...
										["params"] = 
										{
											["action"] = 
											{
												...
												["params"] = 
												{
													["command"] = "Custom_Altitude('" .. grpname .. "',  '  nil  ', '" .. 2 .. "')",
												},
											},
										},
									},
				...
				for i = 1, #units do
					if units[i]["parking_id"] then 
						info05 = info05.."| parking_id: |"..units[i]["parking_id"].."|"						
					end												
				end
...
--change la frequency de certain module, car bien qu'ils ont les radio adequat, le variable frequency est limité
--onlyVariableFrequency
-- local testFreqency = GetFrequency(side, flight[f].target_name, flight[f].task, flight[f].type, nil)
-- for _side, side in pairs(mission.coalition) do	
-- 	for countryN, country in pairs(side.country) do
...
-- 					if frequency[group.units[1].type] and frequency[group.units[1].type].onlyVariableFrequency then
						
-- 						os.execute 'pause'

-- 						if group.frequency >= frequency[group.units[1].type].onlyVariableFrequency.min 
-- 						and group.frequency <= frequency[group.units[1].type].onlyVariableFrequency.max 
-- 						 then
...
-- local camp_str = "assigned_freq_AtoFP = " .. TableSerialization(assigned_freq, 0)						
-- local campFile = io.open("Debug/assigned_freq_AtoFP.lua", "w")								
-- campFile:write(camp_str)															
-- campFile:close()
