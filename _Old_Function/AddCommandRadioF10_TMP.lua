...
local function cleanEWR_list(initiator)
    ...
    if initiator and initiator.getPlayerName and initiator:getPlayerName() then
	...
	if ewr and ewr.players[p] then
		local data = ewr.players[p]

		if data.idOn then
			missionCommands.removeItem(data.idOn)
		end

		if data.idOff then
			missionCommands.removeItem(data.idOff)
		end
	...
	-- === Nettoyage données joueur ===
	EWR_optionPlayer[p] = nil


	-- === Reset throttle pour ce joueur ===
	-- (mais PAS destruction menu groupe)
